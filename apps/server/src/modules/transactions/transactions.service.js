...
export async function getDashboardSummary({ startDate, endDate, location } = {}) {
  ...
  const result = await query(
    `SELECT
       ...
       COALESCE(SUM(payment_transfer), 0)          AS total_transfer,
...
export async function updateTransaction(id, data, requestingUser) {
  ...
  const result = await query(
    `UPDATE transactions SET
       ...
       payment_cash       = COALESCE($10, payment_cash),
       payment_transfer   = COALESCE($11, payment_transfer),
       deposit_cash       = COALESCE($12, deposit_cash),
       deposit_transfer   = COALESCE($13, deposit_transfer),
       marketing_name     = COALESCE($14, marketing_name),
       marketing_fee      = COALESCE($15, marketing_fee),
       ktp_image_url      = COALESCE($16, ktp_image_url),
       transfer_proof_url = COALESCE($17, transfer_proof_url),
       guest_source       = COALESCE($18, guest_source),
       notes              = COALESCE($19, notes),
       checkin_at         = COALESCE($20, checkin_at),
       updated_at         = now()
     WHERE id = $1
     RETURNING *`,
    [
