...
import { sendPushToAudience } from './push.js';
...
export function formatWibDateTime(iso) {
...
export function calcEndAt({ created_at, rental_duration }) {
...
/**
 * Insert dengan deteksi inserted-vs-noop via ON CONFLICT (dedupe_key) DO NOTHING.
...
export async function upsertNotification(payload) {
  ...
  if (res.rows.length > 0) return { id: res.rows[0].id, inserted: true };

  // No-op (dedupe_key sudah ada) → ambil id yang existing agar kontrak return sama.
  if (payload?.dedupe_key) {
    const sel = await query(
      'SELECT id FROM public.notifications WHERE dedupe_key = $1 LIMIT 1',
      [payload.dedupe_key]
    );
    return { id: sel.rows[0]?.id ?? null, inserted: false };
  }
  return { id: null, inserted: false };
}

// Fetch holiday status for a specific date from API
export async function fetchHolidayForDate(year, month, day) {
  try {
 ...
 */
export function detectLongHoliday(holidayMap, fromDate) {
...
/**
 * Port dari api/generate-notifications.js (Vercel handler).
 * Logika bisnis identik; Supabase client diganti parameterized SQL (pg).
 * @param {{ now?: Date, windowMinutes?: number }} [options]
 */
export async function generateNotifications({ now = new Date(), windowMinutes = 5 } = {}) {
  ...
  let txList = [];
  try {
    // Replaces: supabase.from('transactions').select(...).is('checkout_at', null)
    const txRes = await query(
      `SELECT id, created_at, rental_duration, apartment_location, room_number,
              customer_name, user_id, checkout_at, checkin_at
    ...
    for (const tx of txList) {
      ...
      if (diffMs <= 0) {
        ...
        if (tx.user_id) c = await upsertNotification({ type: 'checkout_due', title, body, data: baseData, dedupe_key: `${dedupe}:user:${tx.user_id}`, audience_user_id: tx.user_id });
      ...
      if (diffMs > 0 && diffMs <= cutoffSoonMs) {
        ...
        const a = await upsertNotification({ type: 'checkout_soon', title, body, data: { ...baseData, minutes_left: minutesLeft }, dedupe_key: `${dedupe}:admin`, audience_role: 'admin' });
        const b = await upsertNotification({ type: 'checkout_soon', title, body, data: { ...baseData, minutes_left: minutesLeft }, dedupe_key: `${dedupe}:super_admin`, audience_role: 'super_admin' });
        let c = { inserted: false };
        if (tx.user_id) c = await upsertNotification({ type: 'checkout_soon', title, body, data: { ...baseData, minutes_left: minutesLeft }, dedupe_key: `${dedupe}:user:${tx.user_id}`, audience_user_id: tx.user_id });
        recordInserted(a.inserted); recordInserted(b.inserted); recordInserted(c.inserted);
        if (pushEnabled && (a.inserted || b.inserted || c.inserted)) {
          if (a.inserted) await sendPushToAudience({ title, body, url: '/?tab=kamar', audience_role: 'admin' });
          if (b.inserted) await sendPushToAudience({ title, body, url: '/?tab=kamar', audience_role: 'super_admin' });
  ...
  // ================================================================
  try {
    // Replaces: supabase.from('lokasi_apartemen').select('name').order('name')
    //       dan supabase.from('nomor_kamar').select('name, lokasi').order('name')
    const [locRes, kamarRes] = await Promise.all([
      query('SELECT name FROM public.lokasi_apartemen ORDER BY name'),
      query('SELECT name, lokasi FROM public.nomor_kamar ORDER BY name'),
    ]);
    ...
    const kamar = kamarRes.rows || [];
    const lokasi = (locRes.rows || []).map((x) => x.name);
    ...
    for (const loc of lokasi) {
      ...
      if (availableCount === 0) {
        ...
        const a = await upsertNotification({ type: 'rooms_sold_out', title, body, data: { lokasi: loc, available_count: 0 }, dedupe_key: `${dedupe}:admin`, audience_role: 'admin' });
        const b = await upsertNotification({ type: 'rooms_sold_out', title, body, data: { lokasi: loc, available_count: 0 }, dedupe_key: `${dedupe}:super_admin`, audience_role: 'super_admin' });
        ...
        const a = await upsertNotification({ type: 'rooms_low', title, body, data: { lokasi: loc, available_count: availableCount }, dedupe_key: `${dedupe}:admin`, audience_role: 'admin' });
        const b = await upsertNotification({ type: 'rooms_low', title, body, data: { lokasi: loc, available_count: availableCount }, dedupe_key: `${dedupe}:super_admin`, audience_role: 'super_admin' });
        recordInserted(a.inserted); recordInserted(b.inserted);
        if (pushEnabled && (a.inserted || b.inserted)) {
          if (a.inserted) await sendPushToAudience({ title, body, url: '/?tab=kamar', audience_role: 'admin' });
          if (b.inserted) await sendPushToAudience({ title, body, url: '/?tab=kamar', audience_role: 'super_admin' });
  ...
  // ================================================================
  try {
    // Replaces: supabase.from('tagihan_bulanan').select(...).eq('status','unpaid').lt('due_date', todayStr)
    // due_date::text menjaga format 'YYYY-MM-DD' seperti yang dikembalikan Supabase (pg mengembalikan DATE sebagai JS Date).
    const tagihanRes = await query(
      `SELECT id, apartment_location, room_number, amount, due_date::text AS due_date
         FROM public.tagihan_bulanan
        WHERE status = 'unpaid' AND due_date < $1::date`,
      [todayStr]
    );

    const list = tagihanRes.rows || [];
    result.sections.tagihan.processed = list.length;

    for (const tagihan of list) {
      // 'YYYY-MM-DD' diparse sebagai UTC midnight (Date(STRING)) — perilaku sama seperti
      // saat Supabase mengembalikan due_date sebagai string.
      const dueDate = new Date(`${tagihan.due_date}T00:00:00Z`);
  ...
  try {
    if (dowWib === 5) {
      ...
      const r = await upsertNotification({ type: 'weekend', title, body, data: { date: todayStr }, dedupe_key: `weekend_start:${todayStr}`, audience_role: 'all' });
      recordInserted(r.inserted);
      if (pushEnabled && r.inserted) await sendPushToAudience({ title, body, url: '/', audience_role: 'all' });
